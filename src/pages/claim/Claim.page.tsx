...
import { Avatar, Badge, Button, Center, Container, Input, Modal, Paper, Select, Stack } from '@mantine/core';
...
function ClaimPage() {
  ...
  useEffect(() => {
    ...
    const { claimed, amount, tokenAddress} = await getLinkDetails(moduleAddress, qParams.c ,qParams.i)
    setLinkDetails({ claimed: claimed, amount: amount, token: tokenAddress, chainId: qParams.c});

    setNetwork(`${NetworkUtil.getNetworkById(Number(qParams.c))?.name} ${NetworkUtil.getNetworkById(Number(qParams.c))?.type}`);
      ...
      <div className={classes.homeContainer}>
        <div className={classes.formContainer}>
          ...
          {/* <Badge
            ...
            <h1>You have {linkDetails.amount ? formatEther(linkDetails.amount): 0} {linkDetails.token ?   'ETH' : ''} to claim</h1>
            <p>
   
            </p>
          </div>

          <button type="button" onClick={async () => { await claimLink(queryParams.c, moduleAddress, queryParams.i,  queryParams.p, "0x958543756A4c7AC6fB361f0efBfeCD98E4D297Db" );  }}>Claim</button>
